...
::surInterfaceHeatFvPatchScalarField
(
    const surInterfaceHeatFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::surInterfaceHeatFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    forAll(TFromSub_, facei)
    {
        if(neg(phip[facei]))   //flux subsurface -> surface: fixedValue
        {
            refValue()[facei] = TFromSub_[facei];
...
void Foam::surInterfaceHeatFvPatchScalarField::write
(
    Ostream& os
) const
{

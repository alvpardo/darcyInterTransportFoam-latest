...
#include "incompressibleTwoPhaseMixture_.H"
...
// ADDED
Foam::tmp<Foam::volScalarField>
Foam::incompressibleTwoPhaseMixture::kappa() const
{
    const volScalarField limitedAlpha1
    (
        min(max(alpha1_, scalar(0)), scalar(1))
    ...
    (
        "kappa",
        limitedAlpha1*rho1_*Cp1_*(1/Pr1_)*nuModel1_->nu()
      + (scalar(1) - limitedAlpha1)*rho2_*Cp2_*(1/Pr2_)*nuModel2_->nu()
    );
}


Foam::tmp<Foam::scalarField>
Foam::incompressibleTwoPhaseMixture::kappa(const label patchI) const
{

    return kappa()().boundaryField()[patchI];
}
